    ...
    loader.load().then(() => {
      ...
      if (mapContainerRef.current && geminiResult && landmarks[geminiResult]) {
        ...
        const adjustedCenter = {
          ...
          lat:
            // @ts-ignore
            landmarks[geminiResult].center.lat +
            getRandomArbitrary(-0.0008, 0.0008),
          // @ts-ignore
          lng:
            // @ts-ignore
            landmarks[geminiResult].center.lng +
            getRandomArbitrary(-0.0008, 0.0008),
        };
        ...
        if (navigator.geolocation) {
          navigator.geolocation.watchPosition(
            (position) => {
              ...
              const correctLandmarkCenter: Position = // @ts-ignore
                (landmarks[geminiResult] as Landmark).center;
        ...
        console.error(
          "Invalid geminiResult or landmarks entry missing:",
          geminiResult
        );
      }
    });
      ...
      {win ? (
        <div className="flex justify-center items-center absolute inset-0 z-20 w-screen h-screen bg-black/[0.8]">
          <div className="bg-zinc-900 w-1/2 h-1/2 border border-zinc-400 rounded-lg flex flex-col justify-center items-center">
            ...
            <button
              className="mt-10 py-2 px-4 bg-indigo-500	 hover:from-primary hover:to-cyan-300 text-white font-bold rounded-lg text-lg transition duration-300 ease-in-out transform hover:-translate-y-1"
              onClick={handleClick}
            >
              Play Again
            </button>
